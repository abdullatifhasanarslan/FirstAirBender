...
int main(int argc, char **argv)
{
    ...
    while(ros::ok()){
        ...
        std::cout <<4.0 * atan2(waypoint.translation.y-robot_pose.getOrigin().y(),waypoint.translation.x-robot_pose.getOrigin().x()) << std::endl;
        std::cout << (4.0 * atan2(waypoint.translation.y, waypoint.translation.x)) << std::endl << std::endl;
        ...
        motor_command.linear.x = 0.1 * sqrt(pow(waypoint.translation.x, 2) + pow(waypoint.translation.y, 2));

        motor_command.angular.z = 0.5 * (atan2(waypoint.translation.y-robot_pose.getOrigin().y(),waypoint.translation.x-robot_pose.getOrigin().x()) - robot_theta);
        //motor_command.angular.z = 4.0 * waypoint_theta;
